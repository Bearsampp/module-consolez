-- default script for clink, called by launch.bat when injecting clink

-- At first, load the original clink.lua file
-- this is needed as we set the script path to this dir and therefore the original
-- clink.lua is not loaded.
local clink_lua_file = clink.get_env('CONSOLE_ROOT')..'\\vendor\\clink\\clink.lua'
...
function set_prompt_filter()
    ...
    -- $_$E[1;31m%USERNAME%@%COMPUTERNAME%$S$E[1;32m$P$E[0m$S$_$E[1;30m$$$S$E[0m
    ...
    local console_prompt = "\x1b[1;92;40m{username}@{computername} \x1b[1;95;40m{cwd} {git}{hg} \n\x1b[1;96;40m{dollar} \x1b[0m"
    console_prompt = string.gsub(console_prompt, "{username}", username)
    console_prompt = string.gsub(console_prompt, "{computername}", computername)
    console_prompt = string.gsub(console_prompt, "{cwd}", cwd)
    if env == nil then
        dollar = "$"
    else
        dollar = "("..env..") $"
    end
    clink.prompt.value = string.gsub(console_prompt, "{dollar}", dollar)
end
...
local completions_dir = clink.get_env('CONSOLE_ROOT')..'/vendor/clink-completions/'
